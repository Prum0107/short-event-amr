...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_queries=2, num_encoder_layers=6,
        ...
        self.decoder = TransformerDecoder(decoder_layer, num_decoder_layers, decoder_norm,
                                          ...
                                          r1_enabled=r1_enabled, r1_eps=r1_eps)
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm=None, return_intermediate=False,
        ...
        self.r1_eps = r1_eps
        self.posthoc_trace_references = False
    ...
    def forward(self, tgt, memory,
        ...
        reference_trace = [reference_points.detach().clone()] if self.posthoc_trace_references else None
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if self.bbox_embed is not None:
                ...
                if self.r1_enabled:
                    # R1 changes only normalized width refinement. The center
                    # remains the baseline inverse-sigmoid residual update.
                    center_logit = tmp[..., 0] + inverse_sigmoid(reference_points[..., 0])
                    width = reference_points[..., 1].clamp(min=self.r1_eps, max=1 - self.r1_eps)
                    width_log = torch.log(width) + tmp[..., 1]
                    new_center = center_logit.sigmoid()
                    new_width = torch.exp(width_log).clamp(min=self.r1_eps, max=1 - self.r1_eps)
                    new_reference_points = torch.stack((new_center, new_width), dim=-1)
                else:
                ...
                if reference_trace is not None:
                    reference_trace.append(reference_points.clone())
        ...
        if reference_trace is not None:
            self.posthoc_reference_trace = reference_trace
...
def build_transformer(args):
    return Transformer(
        ...
        activation='prelu',
        r1_enabled=getattr(args, 'r1_enabled', False),
        r1_eps=getattr(args, 'r1_eps', 1e-3),
    )
